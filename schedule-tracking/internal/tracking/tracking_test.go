...
func TestFesoArrivedChecker(t *testing.T) {
	testTable := []struct {
		...
		{infoAboutMoving: []BaseInfoAboutMoving{{Time: time.Now(), OperationName: "Gate out empty for loading", Location: "XIN BA DA", Vessel: ""}, {Time: time.Now(), OperationName: "Gate in empty from consignee", Location: "Honghai", Vessel: ""}, {Time: time.Now(), OperationName: "ETS", Location: "Shanghai", Vessel: "FESCO DALNEGORSK"}, {Time: time.Now(), OperationName: "Load on board", Location: "Honghai", Vessel: "FESCO DALNEGORSK"}}, isArrived: false},
