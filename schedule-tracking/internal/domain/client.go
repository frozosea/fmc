...
func (c *UserClient) MarkBillNoOnTrack(ctx context.Context, userId int64, number string) error {
	ctx, opt := c.token.GenerateGRPCAuthHeader(ctx, c.serverAccessToken)
	if _, err := c.cli.MarkBillNoOnTrack(ctx, &user_pb.AddMarkOnTrackingRequest{UserId: userId, Number: number}, opt); err != nil {
...
func (c *UserClient) MarkContainerOnTrack(ctx context.Context, userId int64, number string) error {
	ctx, opt := c.token.GenerateGRPCAuthHeader(ctx, c.serverAccessToken)
	if _, err := c.cli.MarkContainerOnTrack(ctx, &user_pb.AddMarkOnTrackingRequest{UserId: userId, Number: number}, opt); err != nil {
...
func (c *UserClient) MarkContainerWasArrived(ctx context.Context, userId int64, number string) error {
	ctx, opt := c.token.GenerateGRPCAuthHeader(ctx, c.serverAccessToken)
	if _, err := c.cli.MarkContainerWasArrived(ctx, &user_pb.AddMarkOnTrackingRequest{UserId: userId, Number: number}, opt); err != nil {
...
func (c *UserClient) MarkBillNoWasArrived(ctx context.Context, userId int64, number string) error {
	ctx, opt := c.token.GenerateGRPCAuthHeader(ctx, c.serverAccessToken)
	if _, err := c.cli.MarkBillNoWasArrived(ctx, &user_pb.AddMarkOnTrackingRequest{UserId: userId, Number: number}, opt); err != nil {
...
func (c *UserClient) MarkContainerWasRemovedFromTrack(ctx context.Context, userId int64, number string) error {
	ctx, opt := c.token.GenerateGRPCAuthHeader(ctx, c.serverAccessToken)
	if _, err := c.cli.MarkContainerWasRemovedFromTrack(ctx, &user_pb.AddMarkOnTrackingRequest{UserId: userId, Number: number}, opt); err != nil {
...
func (c *UserClient) MarkBillNoWasRemovedFromTrack(ctx context.Context, userId int64, number string) error {
	ctx, opt := c.token.GenerateGRPCAuthHeader(ctx, c.serverAccessToken)
	if _, err := c.cli.MarkBillNoWasRemovedFromTrack(ctx, &user_pb.AddMarkOnTrackingRequest{UserId: userId, Number: number}, opt); err != nil {
		go c.logger.ExceptionLog(fmt.Sprintf(`mark container with Number %s for user %d no was removed from tracking failed: %s`, number, userId, err.Error()))
		return err
	}
	return nil
