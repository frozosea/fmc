...
func (r *Repository) Add(ctx context.Context, req *BaseTrackReq, isContainer bool) error {
	for _, v := range req.Numbers {
		if !r.checkNumber(ctx, v) {
			_, err := r.db.ExecContext(ctx, `INSERT INTO "tasks" (number,user_id,time,emails,is_container,email_subject) VALUES ($1,$2,$3,$4,$5,$6)`, v, req.UserId, req.Time, pq.Array(req.Emails), isContainer, req.EmailMessageSubject)
			if err != nil {
				return err
			}
		}
...
func (r *Repository) GetAll(ctx context.Context) ([]*TrackingTask, error) {
	...
	rows, err := r.db.QueryContext(ctx, `SELECT number,user_id,time,emails,is_container,email_subject FROM "tasks"`)
	if err != nil {
		return res, err
	}
	var i = 0
	for rows.Next() {
		...
		if scanErr := rows.Scan(&oneItem.Number, &oneItem.UserId, &oneItem.Time, pq.Array(&oneItem.Emails), &oneItem.IsContainer, &oneItem.EmailMessageSubject); scanErr != nil {
			return res, scanErr
		}
...
func (r *Repository) Update(ctx context.Context, req *BaseTrackReq, isContainer bool) error {
	for _, v := range req.Numbers {
		if _, err := r.db.ExecContext(ctx, `UPDATE "tasks" SET number = $1 ,user_id = $2 ,time = $3,emails = $4, is_container = $5,email_subject = $6`, v, req.UserId, req.Time, pq.Array(req.Emails), isContainer, req.EmailMessageSubject); err != nil {
			return err
		}
	}
	return nil
...
func (r *Repository) GetByNumber(ctx context.Context, number string) (*TrackingTask, error) {
	row := r.db.QueryRowContext(ctx, `SELECT number,user_id,time,emails,is_container,email_subject FROM "tasks" AS t WHERE t.number = $1`, number)
	var task TrackingTask
	if err := row.Scan(&task.Number, &task.UserId, &task.Time, pq.Array(&task.Emails), &task.IsContainer, &task.EmailMessageSubject); err != nil {
