...
export function readUnlocodesFromScv() {
    const outputArr = []
    let data = fs.readFileSync(path.resolve(__dirname, "./unlocodes.csv"))
    CSVToArray(data.toString(), ",")
    return CSVToArray(data.toString(), ",")

}

async function prepareDb(source: DataSource) {
    let repo = source.getRepository(Unlocode)
    let unlocodes = readUnlocodesFromScv()
