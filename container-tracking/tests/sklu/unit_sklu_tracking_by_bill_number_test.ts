...
const requestMoch: IRequest<fetchArgs> = {
    async sendRequestAndGetHtml(_: fetchArgs): Promise<string> {
        if (!isContainerInfoAboutMoving) return fs.readFileSync(path.resolve(__dirname, './skluBillNumberInfoAboutMovingWithManyContainersExample.txt')).toString("utf-8")
        else return fs.readFileSync(path.resolve(__dirname, './skluInfoAboutMovingExampleHtml.txt')).toString("utf-8")
    },
...
describe("SKLU track by bill number test", () => {
    ...
    it("SKLU container number parser test", () => {
        let data = fs.readFileSync(path.resolve(__dirname, './skluBillNumberInfoAboutMovingWithManyContainersExample.txt')).toString("utf-8")
